...
Каждый запуск демо, eval и фоновой API-задачи сохраняет JSONL-трейс
в `data/traces/`. В нём видны шаги графа, LLM-запросы, вызовы
инструментов, задержка и токены. Ключи и персональные данные
перед записью удаляются.

Чтобы одновременно видеть шаги в терминале:

```bash
TRACE_VERBOSE=1 UV_CACHE_DIR=/private/tmp/marketplace-agent-uv-cache make demo
```

Для FastAPI используй ту же команду с `make api` вместо `make demo`.
Трейс фоновой задачи имеет имя `content-<job_id>.jsonl` или
`support-<job_id>.jsonl`.

Посмотреть сохранённый трейс в удобном виде:

```bash
.venv/bin/python scripts/trace_view.py data/traces/<имя-файла>.jsonl
```
...
- Подходящая альтернатива через OpenRouter — `openai/gpt-oss-120b`: при тарифе
  $0,03 за 1 млн входных и $0,17 за 1 млн выходных токенов тот же объём стоил бы
  около $0,36 (31 ₽) за 1 000 товаров и $35,99 (3 112 ₽) за 100 000 товаров.
  Это расчёт по опубликованной цене, а не подтверждённый результат качества:
  платную модель нужно отдельно прогнать на golden-наборе.
